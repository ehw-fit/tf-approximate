...
approx_op_lib_file = os.path.join(dir_path, '..', 'build', 'libApproxGPUOpsTF.so')
...
class TestApproxConv2D(object):
    ...
    def __build(self):
        ...
        self.output_ref_tensor = tf.nn.depthwise_conv2d(input_tensor_quant, filter_tensor_quant, self.stride, 'SAME')
        self.output_tensor = self.test_op_module.approx_depthwise_conv2d_with_min_max_vars(input_tensor_quant, filter_tensor_quant,
    ...
    def run(self, device):
        with tf.device('/{}'.format(device)):
            ...
            with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
                ...
                # result = sess.run(self.output_tensor, feed_dict=inputs_dict)
                # print('{}'.format(result))
...
if __name__ == '__main__':
    ...
    test = TestApproxConv2DPerAxis(test_op_module, [1, 256, 256, 3], [3, 3, 3, 4], [1, 1, 1, 1])
    # test = TestApproxConv2D(test_op_module, [1, 256, 256, 3], [3, 3, 3, 4], [1, 1, 1, 1])
