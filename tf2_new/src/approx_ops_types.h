...
struct NullApproxOpType_t {
    ...
    void Update(tensorflow::OpKernelContext *, const T *, const T *) {}
};
...
struct BaseTableApproxOpType_t {
    ...
    struct OpQuantProps_t {
        ...
        const T *pInputCorr;  ///< Input correction coefficients data.
        const T *pFilterCorr; ///< Filter correction coefficients data.
    };
        ...
        , quantMax((1 << 8) - 1)
    {
        ...
        VLOG(2) << "ApproxTF: Loading mul file: " << tableFilename;
    ...
    void UpdateVarQuantProps(float minValue, float maxValue, VarQuantProps_t &quantProps)
    {
        ...
        if(minValue != 0.0f || maxValue != 0.0f)
        {
            tensorflow::Nudge(minValue, maxValue, quantMin, quantMax,
                              &nudgedMin, &nudgedMax, &nudgedScale, &invNudgedScale);
        }
     ...
     * @param ctx
     */
    void Update(tensorflow::OpKernelContext *ctx, const T *pInputCorr, const T *pFilterCorr)
    {
        {
...
//            std::cout << "FILTER_RANGE: " << minValue << " " << maxValue << std::endl;
        ...
        quantProps.pInputCorr = pInputCorr;
        quantProps.pFilterCorr = pFilterCorr;
...
template<typename Device, typename T, typename AT>
struct ApproxFilterCorrCoeff<Device, T, AT, NullApproxOpType_t> {
    void operator()(const Device &d, const NullApproxOpType_t<Device, T, AT> &approxOp,
                    ConstTensor4<T> filterCoeffs, Flat<T> outCorrCoeffs);
};
...
template<typename T>
inline EIGEN_DEVICE_FUNC T Clamp(T value, T minVal, T maxVal) { return std::min(std::max(value, minVal), maxVal); }

template<typename AT, unsigned width, typename T>
inline EIGEN_DEVICE_FUNC AT ClampBitWidth(T value) { return AT(Clamp(value, T(0), T((1 << width) - 1))); }

#ifdef APPROX_OPS_TYPES_H_COMMON_DEFINITIONS

template<typename Device, typename T, typename AT, template<typename, typename, typename> class ApproxOpType>
void ApproxFilterCorrCoeff<Device, T, AT, ApproxOpType>::operator()(const Device &d, const ApproxOpType<Device, T, AT> &approxOp, 
    ConstTensor4<T> filterCoeffs, Flat<T> outCorrCoeffs)
{
...
template<typename Device, typename T, typename AT>
void ApproxFilterCorrCoeff<Device, T, AT, NullApproxOpType_t>::operator()(const Device &d, const NullApproxOpType_t<Device, T, AT> &approxOp, 
    ConstTensor4<T> filterCoeffs, Flat<T> outCorrCoeffs)
{
    outCorrCoeffs.device(d) = outCorrCoeffs.constant(T(0));
}

#endif // ENABLE_DEFINITIONS
