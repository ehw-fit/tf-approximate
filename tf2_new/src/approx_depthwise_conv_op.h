...
#ifndef APPROX_DEPTHWISE_CONV_OP_H
#define APPROX_DEPTHWISE_CONV_OP_H
...
struct LaunchApproxDepthwiseConvOp {
    ...
    void operator()(OpKernelContext* ctx, const ApproxDepthwiseArgs& args,
                    ...
                    TensorFormat data_format, const ApproxOpData &approxOpData);
};

template<typename ApproxOpData>
struct LaunchApproxDepthwiseConvBackpropInputOp {
    typedef typename ApproxOpData::DeviceType_t Device;
    typedef typename ApproxOpData::DataType_t T;
...
template<typename ApproxOpData>
struct LaunchApproxDepthwiseConvBackpropFilterOp {
    typedef typename ApproxOpData::DeviceType_t Device;
    typedef typename ApproxOpData::DataType_t T;
...
struct LaunchApproxDepthwiseConvOp<ApproxOpData<ApproxOpType<Eigen::GpuDevice, T, uint8> > > {
  void operator()(OpKernelContext* ctx, const ApproxDepthwiseArgs& args,
                  const T* input, const T* filter, T* output,
...
template<template<typename> class ApproxOpData, template<typename, typename, typename> class ApproxOpType, typename T>
struct LaunchApproxDepthwiseConvBackpropInputOp<ApproxOpData<ApproxOpType<Eigen::GpuDevice, T, uint8> > > {
  void operator()(class OpKernelContext* ctx, const ApproxDepthwiseArgs& args,
                  const T* out_backprop, const T* filter, T* in_backprop,
                  TensorFormat data_format);
};

template<template<typename> class ApproxOpData, template<typename, typename, typename> class ApproxOpType, typename T>
struct LaunchApproxDepthwiseConvBackpropFilterOp<ApproxOpData<ApproxOpType<Eigen::GpuDevice, T, uint8> > > {
  void operator()(class OpKernelContext* ctx, const ApproxDepthwiseArgs& args,
                  const T* out_backprop, const T* input, T* filter_backprop,
                  TensorFormat data_format);
};
...
template <typename T>
struct ApproxDepthwiseFilterPadOp {
    void operator()(const ApproxDepthwiseArgs& args, const T* filter,
                    T* padded_filter) {
...
template <typename T>
struct ApproxDepthwiseInputCopyOp {
    void operator()(const ApproxDepthwiseArgs& args,
...
#endif // APPROX_DEPTHWISE_CONV_OP_H
