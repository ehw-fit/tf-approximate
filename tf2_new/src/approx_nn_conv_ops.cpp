...
struct LaunchGeneric {
    void operator()(OpKernelContext *ctx, const Tensor &input, const Tensor &filter,
                    ...
                    TensorFormat dataFormat, ApproxOpType<Device, T, AT> &approxOp)
    {
        typedef ApproxConvOpQuantData<ApproxOpType<Device, T, AT> > ApproxConvOpData_t;

        std::cout << "ApproxConvCPU" << std::endl;

        CHECK(dataFormat == FORMAT_NHWC) << "Generic conv implementation only "
                                            "supports NHWC tensor format for now.";
        
        ApproxConvOpData_t quantData(ctx, approxOp);
                ...
                colStride == 1 && (padding == SAME || padding == VALID))
        {
            ...
            ApproxConvGEMMKernelCombined<ApproxConvOpData_t>()(
                        ctx->eigen_device<Device>(), quantData,
                        m, n, k, T(0),
        ...
        {
            std::cout << "COMPUTE_FULL" << std::endl;
...

            {
                ...
                OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<AT>::v(), im2ColTmpBufferShape, &im2ColTmpBuffer));
            }

            AT *pIm2ColBufferData = im2ColTmpBuffer.flat<AT>().data();

            quantData.AllocateInputCorrCoefficients(patchesPerChunk);
            ...
            else
            {
                ...
                for(int64 i = 0; i < totalPatchesCount; i += patchesPerChunk)
                {
                    ...
                    ApproxConvIm2ColKernel<ApproxConvOpData_t>()(
                                ctx->eigen_device<Device>(), quantData,
                                pInputData,
                                ...
                                colDilation, rowDilation, patchOffset, patchesCount, pIm2ColBufferData);
                    ...
                    ApproxConvGEMMKernel<ApproxConvOpData_t>()(
                                ctx->eigen_device<Device>(), quantData,
                                m, n, k, T(0),
...
struct LaunchApproxConv2DOp<CPUDevice, T, AT, ApproxOpType> {
    void operator()(OpKernelContext *ctx,
                    ...
                    TensorFormat dataFormat, ApproxOpType<CPUDevice, T, AT> &approxOp)
    {
...
void LaunchApproxConv2DOp<GPUDevice, T, AT, ApproxOpType>::operator()(OpKernelContext *ctx,
                            ...
                            int rowDilation, int colDilation, int rowStride, int colStride,
                            const Padding &padding, const std::vector<int64> &explicitPaddings, Tensor *output,
                            TensorFormat dataFormat, ApproxOpType<GPUDevice, T, AT> &approxOp)
{
    typedef ApproxConvOpQuantData<ApproxOpType<GPUDevice, T, AT> > ApproxConvOpData_t;
            ...
            (padding == VALID || padding == SAME))
    {
        ...
        ApproxConvGEMMKernelCombined<ApproxConvOpData_t>()(ctx->eigen_device<GPUDevice>(), quantData,
                                                                       m, n, k, T(0),
                                                                       input.flat<T>().data(), k,
    ...
    else
    {
...

        {
            ...
            OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<AT>::v(), shape, &im2ColTmpBuffer));
        }
        ...
        else
        {
            ...
            for(int64 i = 0; i < totalPatchesCount; i += patchesPerChunk)
            {
                ...
                ApproxConvIm2ColKernel<ApproxConvOpData_t>()(
                            ctx->eigen_device<GPUDevice>(), quantData,
