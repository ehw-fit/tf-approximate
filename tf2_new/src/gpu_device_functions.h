...
template <typename F>
__device__ long long GpuAtomicCasHelper(tensorflow::uint64* ptr, F accumulate) {
...
__device__ inline uint64 GpuAtomicAdd(tensorflow::uint64* ptr, tensorflow::uint64 value) {
  ...
  return atomicAdd(reinterpret_cast<unsigned long long *>(ptr), static_cast<unsigned long long>(value));
}
